...
void main(void)
{
	...
	float cosTheta = dot(nN,Vi); // vu que les deux vecteurs sont normalisés alors le produit scalaire = cos(theta)
	...
	float cosAlpha = dot(normalize(R),Vo); //cosinus de l'angle alpha (entre le vecteur réfléchi et le vecteur de vision)
